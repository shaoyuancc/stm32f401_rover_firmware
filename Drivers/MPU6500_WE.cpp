...
/************  Constructors ************/
// Provide 7-bit device address
MPU6500_WE::MPU6500_WE(I2C_HandleTypeDef * i2cHandle, uint8_t addr)
...
float MPU6500_WE::getResultantG(xyzFloat gVal){
    float resultant = 0.0;
    resultant = std::sqrt(gVal.x*gVal.x + gVal.y*gVal.y + gVal.z*gVal.z);
...
std::string MPU6500_WE::getOrientationAsString(){
...
void MPU6500_WE::writeMPU9250Register(uint8_t reg, uint8_t val){
    HAL_I2C_Mem_Write(_i2cHandle, i2cAddress, reg, 1, &val, 1, I2C_TIMEOUT);
...
uint8_t MPU6500_WE::readMPU9250Register8(uint8_t reg){
    uint8_t regValue = 0;
    HAL_I2C_Mem_Read(_i2cHandle, i2cAddress, reg, 1, &regValue, 1, I2C_TIMEOUT);
    return regValue;
//    if(!useSPI){
//    }
//    else{
...
int16_t MPU6500_WE::readMPU9250Register16(uint8_t reg){
    ...
    if (HAL_I2C_Mem_Read(_i2cHandle, i2cAddress, reg, 1, tempData, 2, I2C_TIMEOUT) == HAL_OK){
        regValue = ((uint8_t) tempData[0]<<8) + (uint8_t) tempData[1];
    }
...
uint64_t MPU6500_WE::readMPU9250Register3x16(uint8_t reg){
    ...
    if (HAL_I2C_Mem_Read(_i2cHandle, i2cAddress, reg, 1, mpu9250Triple, 6, I2C_TIMEOUT) == HAL_OK){
...
xyzFloat MPU6500_WE::readMPU9250xyzValFromFifo(){
    ...
    if (HAL_I2C_Mem_Read(_i2cHandle, i2cAddress, REGISTER_FIFO_R_W, 1, fifoTriple, 6, I2C_TIMEOUT) == HAL_OK){
